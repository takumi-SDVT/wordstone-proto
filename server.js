...
export async function handler(event) {
  ...
  try {
    const { topic, backKind, extraKind, difficulty, count } =
      ...
      const messages = [
        {
          role: "system",
          content:
            'You generate concise study flashcards. Output strictly in JSON, no extra text, no Markdown, no code fences. Schema: {"cards":[{"front":string,"back":string,"extra":string}]}',
        },
        {
          role: "user",
...
出力は自由な文章（形式に制限されない充実した内容）をJSON形式（{"cards":[{"front":string,"back":string,"extra":string}]})で、まとめてください。`,
      ...
      if (existingTerms.includes(card.front)) { i--; continue; }
