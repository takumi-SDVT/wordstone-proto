...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    ...
    // ドロップダウンの値変更時にカスタム入力欄を制御
    document.getElementById('backKind').addEventListener('change', () => {
      document.getElementById('backKind-custom').style.display = document.getElementById('backKind').value === '任意' ? 'block' : 'none';
    });
    document.getElementById('extraKind').addEventListener('change', () => {
      document.getElementById('extraKind-custom').style.display = document.getElementById('extraKind').value === '任意' ? 'block' : 'none';
    });
    ...
    document.getElementById('sheet').addEventListener('touchmove', e => {
      ...
      if (dy > 0) document.getElementById('sheet').style.transform = `translateY(${Math.min(dy, 160)}px)`;
    }, { passive: true });
    document.getElementById('sheet').addEventListener('touchend', () => {
      const t = parseInt(document.getElementById('sheet').style.transform.replace(/[^\-0-9]/g, '')) || 0;
      if (t > 120) closeModal();
      else document.getElementById('sheet').style.transform = 'translateY(0)';
      startY = null;
    });
    ...
    // フォルダデータの置き場（selectedFolderId を追加）
    const folders = [];
    let selectedFolderId = null;

    // フォルダをlocalStorageに保存
    function saveFolders() {
      console.log('Saving folders:', folders);
      try {
        localStorage.setItem('folders', JSON.stringify(folders));
      } catch (e) {
        console.error('Save folders failed:', e);
        showToast('フォルダの保存に失敗しました');
      }
    }

    // フォルダをlocalStorageから読み込み
    function loadFolders() {
      try {
        ...
        if (data) {
          const parsed = JSON.parse(data);
          folders.length = 0;
          parsed.forEach(folder => folders.push(folder));
        }
      } catch (e) {
        console.error('Load folders failed:', e);
        folders.length = 0;
      }
      selectedFolderId = null; // 初期化
      renderFolderView();
    }
    ...
    function renderFolderView() {
      ...
      const backBtn = document.getElementById('folderView').querySelector('.back-to-folders-btn');
      folderListEl.innerHTML = '';
      deckListEl.innerHTML = '';
      folderListEl.style.display = selectedFolderId ? 'none' : 'block';
      deckListEl.style.display = selectedFolderId ? 'block' : 'none';
      backBtn.style.display = selectedFolderId ? 'block' : 'none';

      if (!selectedFolderId) {
        // フォルダ一覧表示
        if (folders.length === 0) {
          folderListEl.innerHTML = '<p class="muted">フォルダがありません。新規フォルダを作成してください。</p>';
          return;
        }
        for (const folder of folders) {
          const item = document.createElement('div');
          item.className = 'folder-item' + (folder.id === selectedFolderId ? ' selected' : '');
          item.innerHTML = `
            <div>
              <span class="folder-item-title">${escapeHTML(folder.name)}</span>
              <input class="folder-name-input" value="${escapeAttr(folder.name)}" style="display: none;">
            </div>
            <div style="display: flex; gap: 8px;">
              <button class="btn btn-edit-folder">編集</button>
        ...
        if (!folder || folder.deckIds.length === 0) {
          deckListEl.innerHTML = '<p class="muted">単語帳がありません。「フォルダに追加」から追加してください。</p>';
          return;
        }
        for (const [index, deckId] of folder.deckIds.entries()) {
          ...
          item.innerHTML = `
            <div>
              ...
              <div class="deck-item-meta">最終使用: ${deck.lastUsed ?? '未使用'} | 進捗: ${deck.progress ? `${Math.round(deck.progress.correct / deck.progress.total * 100)}% (${deck.progress.correct}/${deck.progress.total})` : '未学習'}</div>
            </div>
            <div style="display: flex; gap: 8px;">
              <button class="btn btn-move-up" ${index === 0 ? 'disabled' : ''}>↑</button>
              <button class="btn btn-move-down" ${index === folder.deckIds.length - 1 ? 'disabled' : ''}>↓</button>
              <button class="btn btn-danger">削除</button>
            </div>
          `;
          item.querySelector('.deck-item-title').addEventListener('click', () => go(`#/deck/${deck.id}`));
    ...
    document.getElementById('folderView').addEventListener('click', (e) => {
      ...
      const deleteFolderBtn = e.target.closest('.folder-item .btn-danger');
      const backBtn = e.target.closest('.back-to-folders-btn');
      const moveUpBtn = e.target.closest('.btn-move-up');
      const moveDownBtn = e.target.closest('.btn-move-down');
      const deleteDeckBtn = e.target.closest('.deck-item .btn-danger');
      ...
      if (newFolderBtn) {
        ...
        renderFolderView();
      } else if (editBtn && folderItem) {
        const index = Array.from(document.getElementById('folderList').children).indexOf(folderItem);
        const titleEl = folderItem.querySelector('.folder-item-title');
        const inputEl = folderItem.querySelector('.folder-name-input');
        titleEl.style.display = 'none';
        inputEl.style.display = 'block';
        inputEl.focus();
      } else if (deleteFolderBtn && folderItem) {
        const index = Array.from(document.getElementById('folderList').children).indexOf(folderItem);
        if (confirm(`フォルダ「${folders[index].name}」を削除しますか？`)) {
        ...
        selectedFolderId = null;
        renderFolderView();
      } else if (folderItem && e.target.classList.contains('folder-item-title')) {
        const index = Array.from(document.getElementById('folderList').children).indexOf(folderItem);
        selectedFolderId = folders[index].id;
        renderFolderView();
      } else if (moveUpBtn) {
        const deckItem = moveUpBtn.closest('.deck-item');
        const index = Array.from(document.getElementById('deckList').children).indexOf(deckItem);
        const folder = folders.find(f => f.id === selectedFolderId);
        if (index > 0) {
          [folder.deckIds[index - 1], folder.deckIds[index]] = [folder.deckIds[index], folder.deckIds[index - 1]];
          saveFolders();
          renderFolderView();
          showToast('単語帳を上に移動しました');
        }
      } else if (moveDownBtn) {
        const deckItem = moveDownBtn.closest('.deck-item');
        const index = Array.from(document.getElementById('deckList').children).indexOf(deckItem);
        const folder = folders.find(f => f.id === selectedFolderId);
        if (index < folder.deckIds.length - 1) {
          [folder.deckIds[index], folder.deckIds[index + 1]] = [folder.deckIds[index + 1], folder.deckIds[index]];
          saveFolders();
          renderFolderView();
          showToast('単語帳を下に移動しました');
        }
      } else if (deleteDeckBtn) {
        const deckItem = deleteDeckBtn.closest('.deck-item');
        const index = Array.from(document.getElementById('deckList').children).indexOf(deckItem);
        const folder = folders.find(f => f.id === selectedFolderId);
        if (confirm(`この単語帳をフォルダから削除しますか？`)) {
          folder.deckIds.splice(index, 1);
          saveFolders();
          showToast('単語帳をフォルダから削除しました');
          renderFolderView();
        }
      }
    });
    ...
    // フォルダ名編集ハンドラ（変更なし）
    document.getElementById('folderView').addEventListener('input', (e) => {
      const inputEl = e.target.closest('.folder-name-input');
      if (inputEl) {
        const folderItem = inputEl.closest('.folder-item');
        const index = Array.from(document.getElementById('folderList').children).indexOf(folderItem);
        folders[index].name = inputEl.value;
        saveFolders();
        showToast('フォルダ名を更新しました');
      }
    });

    // フォルダ名編集確定（Enter/Blur、変更なし）
    document.getElementById('folderView').addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && e.target.classList.contains('folder-name-input')) {
        const folderItem = e.target.closest('.folder-item');
        const titleEl = folderItem.querySelector('.folder-item-title');
        const inputEl = folderItem.querySelector('.folder-name-input');
    ...
    document.getElementById('folderView').addEventListener('blur', (e) => {
    ...
    // フォルダ追加ハンドラ（スクリプトの最後に追加）
    document.getElementById('folderAddForm').addEventListener('click', (e) => {
      const addBtn = e.target.closest('#addToFolderBtn');
      if (addBtn) {
        const folderSelect = document.getElementById('folderSelect');
        const folderId = folderSelect.value;
        const deckId = document.getElementById('folderAddForm').dataset.deckId;
        const folder = folders.find(f => f.id === folderId);
        if (folder) {
          if (folder.deckIds.includes(deckId)) {
            showToast('この単語帳はすでにフォルダに追加されています');
            return;
          }
          folder.deckIds.push(deckId);
          saveFolders();
          showToast('フォルダに追加しました');
          closeModal();
        }
      }
    });
